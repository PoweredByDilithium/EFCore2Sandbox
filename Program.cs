  ...
  class Program {
    ...
    static void DataEntryMenu () {
      ...
      do {
        ...
        } catch (System.FormatException) { // a key that wasn't a number
        }
      } while (!cont);
    }

    static void DisplayAllMachines () {
    ...
    static bool ValidateYorN (string entry) {
      bool result = false;
      if (entry.ToLower ().Trim () == "y" || entry.ToLower ().Trim () == "n") {
        result = true;
      }
      return result;
    }
    ...
    static bool CheckForExistingOS (string osName) {
      ...
      using (var context = new MachineContext ()) {
        var os = context.OperatingSys.Where (o => o.Name.Trim () == osName.Trim ());
        if (os.Count () > 0) {
          exists = true;
        }
      }
    ...
    static void SelectOperatingSystem (string operation) {
      ...
      do {
        ...
        if (cki.Key == ConsoleKey.Escape) {
          ...
          } catch (System.ArgumentOutOfRangeException) { // at the 0 position, can't go any further back
          }
        } else {
          if (char.IsNumber (cki.KeyChar)) {
